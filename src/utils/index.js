...
export const organizeFormatTree = (data, valueField, lableField, childField, type, companyField) => {
  let tree = [];
  data.forEach((node, index) => {
    let newNode = {
      value: node[valueField],
      label: node[lableField],
      companyId: node[companyField]
    };
    if (type) {
    ...
    tree.push(newNode);
    if (node[childField] != null && node[childField].length > 0) {
      newNode.children = (organizeFormatTree(node[childField], valueField, lableField, childField,
        type));
    }
  }, this);
  return tree;
