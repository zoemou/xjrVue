<template>
    ...
    <main-content :title="contentTitle" >
        <div slot="search">
             ...
             <div class="searchBox">
                <el-form label-width="70px">
                    <el-form-item  label="开票编号" style="display: inline-block;">
						<el-input v-model="searchParam.F_SalesInvoiCode" size="small" placeholder="请输入开票编号查询"   style="width:200px" ></el-input>
                     </el-form-item>
                     <el-form-item  label="销售订单" style="display: inline-block; margin-right: 5px;">
						<el-select v-model="searchParam.F_SalesCode" filterable clearable style="width:200px"  placeholder="请选择">
							<el-option  v-for="(item,index) in F_SalesCode_Option"  :key="index" :label="item.F_SalesCode" :value="item.F_SalesCode" >  </el-option>
						</el-select> 
                     </el-form-item>
        ...
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="ERP_SalesInvoiceTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <ERP_SalesInvoiceForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></ERP_SalesInvoiceForm>
            </el-dialog>
        </div>
    </main-content>
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import ERP_SalesInvoiceForm from './ERP_SalesInvoiceForm';
     export default {
         name: "ERP_SalesInvoice",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,ERP_SalesInvoiceForm,createFlow },
         data() {
             ...
             F_SalesCode_Option : [],                   option:{
                   ...
                   created() {
                this.getF_SalesCode_Option();                   },
				mounted(){
					  let themeName = getStore({ name: "themeName" }) || "default";
					  if((themeName == "eveningTheme" && window.screen.width<1214)||(themeName != "eveningTheme" &&window.screen.width<1214)){
	...
	f.F_TaxCode from erp_salesorder o left join erp_inoutunit f on o.F_ClientId=f.f_id`
                 ...
                 this.F_SalesCode_Option = res.data.data 
             }) 
          }, 
                         /* 获取公司用户数据 */
                         getListData() {
                            request({
                                url: baseUrl + '/erp_salesinvoice',
                                method: 'get',
                         ...
                         searchClick() {
                          ...
                          /* 查询表单重置 */
                         searchReset() {
                       this.searchParam.F_SalesInvoiCode = null;
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/erp_salesinvoice/" + val.F_Id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/erp_salesinvoice/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
