<template>
    ...
    <main-content :title="contentTitle" >
        <div slot="search">
             ...
             <div class="searchBox">
                <el-form label-width="70px" style="display: inline-block;margin-right: 70px;">
                    <el-form-item  label="入库编号"style="display: inline-block;">
						<el-input v-model="searchParam.F_InStockCode" size="small" placeholder="请输入入库编号查询"   style="width:200px" ></el-input>
                    </el-form-item>
                    <el-form-item  label="入库日期" style="display: inline-block;">
                        <el-date-picker size="small"  type="daterange" value-format="yyyy-MM-dd" v-model="F_InStockDate_Date"   style="width:250px" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="F_InStockDate_DateChange"> </el-date-picker>
                    </el-form-item>
        ...
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="ERP_InStockTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <ERP_InStockForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></ERP_InStockForm>
            </el-dialog>
        </div>
    </main-content>
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import ERP_InStockForm from './ERP_InStockForm';
     export default {
         name: "ERP_InStock",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,ERP_InStockForm,createFlow },
         data() {
             ...
             F_WarehouseId_Option : [],                   option:{
				...
				mounted(){
					  ...
					  if((themeName == "eveningTheme" && window.screen.width<1514)||(themeName != "eveningTheme" &&window.screen.width<1514)){
	...
	erp_warehouse w`
                 ...
                 this.F_WarehouseId_Option = res.data.data 
                         ...
                         searchClick() {
                          ...
                          /* 查询表单重置 */
                         searchReset() {
                       this.searchParam.F_InStockCode = null;
                       this.searchParam.F_InStockDate_Start = null;
                       this.searchParam.F_InStockDate_End = null;
                       this.searchParam.F_InStockDate_Date = null;
                       this.F_InStockDate_Date = [];
...
                           }
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/erp_instock/" + val.F_Id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/erp_instock/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
                   ...
                   F_InStockDate_DateChange(val){
                       this.searchParam.F_InStockDate_Start = val[0];
                       this.searchParam.F_InStockDate_End = val[1];
                   },
         }
     }
</script>
