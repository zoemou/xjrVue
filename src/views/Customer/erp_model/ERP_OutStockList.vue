<template>
    ...
    <main-content :title="contentTitle" >
             ...
             <div class="searchBox">
                <el-form label-width="70px" style="display: inline-block; margin-right: 70px;">
                    <el-form-item  label="出库日期"style="display: inline-block;">
                        <el-date-picker size="small"  type="daterange" value-format="yyyy-MM-dd" v-model="F_OutStockDate_Date"   style="width:250px" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="F_OutStockDate_DateChange"> </el-date-picker>
                    </el-form-item>
                    ...
                    <el-form-item  label="出库编号"style="display: inline-block; margin-right: 5px;">
						<el-input v-model="searchParam.F_OutStockCode" size="small" placeholder="请输入出库编号查询"   style="width:200px" ></el-input>
                    </el-form-item>
        ...
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="ERP_OutStockTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <ERP_OutStockForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></ERP_OutStockForm>
            </el-dialog>
      <el-dialog title="快速导入" width="400px" :append-to-body="true" :visible.sync="excelDialog">
        <el-upload
           ...
           :action="baseUrl + '/erp_outstock/import'">
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import ERP_OutStockForm from './ERP_OutStockForm';
     export default {
         name: "ERP_OutStock",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,ERP_OutStockForm,createFlow },
         data() {
             ...
             F_WarehouseId_Option : [],                   option:{
                   ...
                   created() {
                this.getF_WarehouseId_Option();                      const token = JSON.parse(localStorage.getItem('avue-token'))['content'];
                      this.headers.Authorization = 'Bearer ' + token
                       this.uploadData = { F_ModuleId: this.$route.meta.moduleid };
                   },
				   mounted(){
				   	  let themeName = getStore({ name: "themeName" }) || "default";
				   	  if((themeName == "eveningTheme" && window.screen.width<1660)||(themeName != "eveningTheme" &&window.screen.width<1660)){
	...
	erp_warehouse w`
                 ...
                 this.F_WarehouseId_Option = res.data.data 
                         ...
                         download(){
                             request({
                                 ...
                                 responseType: 'blob'
                                    ...
                                    link.href = url;
                                    link.setAttribute('download', 'ERP_OutStock.xlsx');
                         ...
                         searchClick() {
                          ...
                          /* 查询表单重置 */
                         searchReset() {
                       this.searchParam.F_OutStockDate_Start = null;
                       this.searchParam.F_OutStockDate_End = null;
                       this.searchParam.F_OutStockDate_Date = null;
                       this.F_OutStockDate_Date = [];
                       this.searchParam.F_WarehouseId = null;
                       this.searchParam.F_OutStockCode = null;
                            this.getListData()
                         },
                          /*查看*/
                         viewForm(val) {
...
                           }
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/erp_outstock/" + val.F_Id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                         ...
                         sortChange(data) {
                          ...
                          /* 导出 */
                          tableExport(){
                            this.$refs["ERP_OutStockTable"].rowExcel()
                            return;
                         },
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/erp_outstock/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
                   ...
                   F_OutStockDate_DateChange(val){
                       this.searchParam.F_OutStockDate_Start = val[0];
                       this.searchParam.F_OutStockDate_End = val[1];
                   },
         }
     }
</script>
