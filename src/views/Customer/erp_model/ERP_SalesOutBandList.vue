<template>
    ...
    <main-content :title="contentTitle" >
        <div slot="search">
             ...
             <div :class="[!searchFlag ? 'searchBox' : '']">
                <el-form label-width="70px" style="display: inline-block; margin-right: 70px;">
                    <el-form-item  label="出库日期" style="display: inline-block;">
                        <el-date-picker size="small"  type="daterange" value-format="yyyy-MM-dd" v-model="F_SalesOutDate_Date"   style="width:250px" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="F_SalesOutDate_DateChange"> </el-date-picker>
                    </el-form-item>
                    <el-form-item  label="出库单号" style="display: inline-block;">
						<el-input v-model="searchParam.F_SalesOutCode" size="small" placeholder="请输入销售出库单号查询"   style="width:200px" ></el-input>
                    </el-form-item>
                    <el-form-item  label="销售订单" style="display: inline-block;">
						<el-select v-model="searchParam.F_SalesOrderId" filterable clearable style="width:200px"  placeholder="请选择">
							<el-option  v-for="(item,index) in F_SalesOrderId_Option"  :key="index" :label="item.F_SalesCode" :value="item.F_Id" >  </el-option>
						</el-select> 
                   </el-form-item>
				   <el-form-item  label="默认仓库" style="display: inline-block;">
						<el-select v-model="searchParam.F_WarehouseId" filterable clearable style="width:200px"  placeholder="请选择">
							<el-option  v-for="(item,index) in F_WarehouseId_Option"  :key="index" :label="item.F_FullName" :value="item.F_Id" >  </el-option>
						</el-select> 
				   </el-form-item>
        ...
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="ERP_SalesOutBandTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <ERP_SalesOutBandForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></ERP_SalesOutBandForm>
            </el-dialog>
      <el-dialog title="快速导入" width="400px" :append-to-body="true" :visible.sync="excelDialog">
        <el-upload
           ...
           :action="baseUrl + '/erp_salesoutband/import'">
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import ERP_SalesOutBandForm from './ERP_SalesOutBandForm';
     export default {
         name: "ERP_SalesOutBand",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,ERP_SalesOutBandForm,createFlow },
         data() {
			  ...
			  btns:[
                   ...
                   defaultProps: {
                    ...
                    headers: {   },
                    excelDialog: false,
             F_SalesOrderId_Option : [],             F_WarehouseId_Option : [],                   option:{
                       rowKey:'F_Id',
                   ...
                   created() {
                this.getF_SalesOrderId_Option();                this.getF_WarehouseId_Option();                      const token = JSON.parse(localStorage.getItem('avue-token'))['content'];
                      this.headers.Authorization = 'Bearer ' + token
                       this.uploadData = { F_ModuleId: this.$route.meta.moduleid };
                   },
				   mounted(){
				   	  let themeName = getStore({ name: "themeName" }) || "default";
				   	  if((themeName == "eveningTheme" && window.screen.width<1560)||(themeName != "eveningTheme" &&window.screen.width<1750)){
...
`
                 ...
                 this.F_SalesOrderId_Option = res.data.data 
             }) 
          }, 
             ...
             request({ 
                 ...
                 this.F_WarehouseId_Option = res.data.data 
                         ...
                         download(){
                             request({
                                 ...
                                 responseType: 'blob'
                                    ...
                                    link.href = url;
                                    link.setAttribute('download', 'ERP_SalesOutBand.xlsx');
                         ...
                         searchClick() {
                          ...
                          /* 查询表单重置 */
                         searchReset() {
                       this.searchParam.F_SalesOutDate_Start = null;
                       this.searchParam.F_SalesOutDate_End = null;
                       this.searchParam.F_SalesOutDate_Date = null;
                       this.F_SalesOutDate_Date = [];
                       this.searchParam.F_SalesOutCode = null;
                       this.searchParam.F_SalesOrderId = null;
...
                           }
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/erp_salesoutband/" + val.F_Id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                         ...
                         sortChange(data) {
                          ...
                          /* 导出 */
                          tableExport(){
                            this.$refs["ERP_SalesOutBandTable"].rowExcel()
                            return;
                         },
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/erp_salesoutband/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
                   ...
                   F_SalesOutDate_DateChange(val){
                       this.searchParam.F_SalesOutDate_Start = val[0];
                       this.searchParam.F_SalesOutDate_End = val[1];
                   }
         }
     }
</script>
