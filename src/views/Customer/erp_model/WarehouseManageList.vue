<template>
    <div class="main-container padding:0 8px;">
    <main-content :title="contentTitle" >
        <div slot="search">
              <!-- 在此处放置搜索内容 -->
             <div class="searchBox">
                <el-form label-width="70px" style="display: inline-block; margin-right: 70px;">
                    <el-form-item  label="仓库编号"style="display: inline-block;">
						<el-input v-model="searchParam.F_EnCode" size="small" placeholder="请输入仓库编号查询"   style="width:200px" ></el-input>
                    </el-form-item>
                    <el-form-item  label="仓库名称" style="display: inline-block; margin-right: 5px;">
						<el-input v-model="searchParam.F_FullName" size="small" placeholder="请输入仓库名称查询"   style="width:200px" ></el-input>
                    </el-form-item>
					<el-button type="primary" icon="el-icon-search" size="small" style="margin-top: 4px;" @click="searchClick">搜索</el-button>
					<el-button icon="el-icon-refresh" size="small" style="margin-top: 4px;" @click="searchReset">重置</el-button>
                </el-form>
			</div>
        </div>
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="WarehouseManageTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <WarehouseManageForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></WarehouseManageForm>
            </el-dialog>
        </div>
    </main-content>
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import WarehouseManageForm from './WarehouseManageForm';
     export default {
         name: "WarehouseManage",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,WarehouseManageForm,createFlow },
         data() {
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/warehousemanage/" + val.F_Id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/warehousemanage/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
