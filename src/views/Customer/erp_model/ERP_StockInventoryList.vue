<template>
    ...
    <main-content :title="contentTitle" >
        <div slot="search">
              <!-- 在此处放置搜索内容 -->
             <div class="searchBox">
                <el-form  style="display: inline-block; margin-right: 70px;">
                    <el-form-item  label="库存盘点编号" label-width="90px" style="display: inline-block;">
						<el-input v-model="searchParam.F_InventoryCode" size="small" placeholder="请输入库存盘点编号查询"   style="width:200px" ></el-input>
                    </el-form-item>
                    <el-form-item  label="盘点时间"label-width="70px" style="display: inline-block;">
                        <el-date-picker size="small"  type="daterange" value-format="yyyy-MM-dd" v-model="F_InventoryDate_Date"   style="width:250px" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="F_InventoryDate_DateChange"> </el-date-picker>
                    </el-form-item>
                    <el-form-item  label="仓库"label-width="50px" style="display: inline-block; margin-right: 5px;">
        ...
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="ERP_StockInventoryTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <ERP_StockInventoryForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></ERP_StockInventoryForm>
            </el-dialog>
      <el-dialog title="快速导入" width="400px" :append-to-body="true" :visible.sync="excelDialog">
        <el-upload
           ...
           :action="baseUrl + '/erp_stockinventory/import'">
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import ERP_StockInventoryForm from './ERP_StockInventoryForm';
     export default {
         name: "ERP_StockInventory",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,ERP_StockInventoryForm,createFlow },
         data() {
             ...
             F_WareHouseId_Option : [],                   option:{
                ...
                this.getF_WareHouseId_Option();                      const token = JSON.parse(localStorage.getItem('avue-token'))['content'];
				   ...
				   mounted(){
				   	  ...
				   	  if((themeName == "eveningTheme" && window.screen.width<1680)||(themeName != "eveningTheme" &&window.screen.width<1680)){
...
FROM
	erp_warehouse w`
                 ...
                 this.F_WareHouseId_Option = res.data.data 
                         ...
                         download(){
                             request({
                                 ...
                                 responseType: 'blob'
                                    ...
                                    link.href = url;
                                    link.setAttribute('download', 'ERP_StockInventory.xlsx');
                         ...
                         getListData() {
                            request({
                                url: baseUrl + '/erp_stockinventory',
                                method: 'get',
...
                           }
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/erp_stockinventory/" + val.F_Id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                         ...
                         sortChange(data) {
                          ...
                          /* 导出 */
                          tableExport(){
                            this.$refs["ERP_StockInventoryTable"].rowExcel()
                            return;
                         },
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/erp_stockinventory/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
