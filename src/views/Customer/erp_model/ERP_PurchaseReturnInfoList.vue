<template>
    ...
    <main-content :title="contentTitle" >
             ...
             <div :class="[!searchFlag ? 'searchBox' : '']">
                <el-form label-width="70px" style="display: inline-block; margin-right: 70px;">
                  <el-form-item  label="退货单号" style="display: inline-block;">
					<el-input v-model="searchParam.F_ReturnNumber" size="small" placeholder="请输入退货单号查询"   style="width:200px" ></el-input>
				  </el-form-item>
                  <el-form-item  label="退货日期" style="display: inline-block;">
                    <el-date-picker size="small"  type="daterange" value-format="yyyy-MM-dd" v-model="F_ReturnDate_Date"   style="width:250px" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="F_ReturnDate_DateChange"> </el-date-picker>
                  </el-form-item>
                  <el-form-item  label="入库单号" style="display: inline-block;">
					<el-select v-model="searchParam.F_PurReceiptNumber" filterable clearable style="width:200px"  placeholder="请选择">
						<el-option  v-for="(item,index) in F_PurReceiptNumber_Option"  :key="index" :label="item.F_ReceiptNumber" :value="item.F_Id" >  </el-option>
					</el-select> 
                   </el-form-item>
        ...
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="ERP_PurchaseReturnInfoTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <ERP_PurchaseReturnInfoForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></ERP_PurchaseReturnInfoForm>
            </el-dialog>
      <el-dialog title="快速导入" width="400px" :append-to-body="true" :visible.sync="excelDialog">
        <el-upload
           ...
           :action="baseUrl + '/erp_purchasereturninfo/import'">
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import ERP_PurchaseReturnInfoForm from './ERP_PurchaseReturnInfoForm';
     export default {
         name: "ERP_PurchaseReturnInfo",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,ERP_PurchaseReturnInfoForm,createFlow },
         data() {
			  ...
			  btns:[
                   ...
                   defaultProps: {
                    ...
                    headers: {   },
                    excelDialog: false,
             F_PurReceiptNumber_Option : [],                   option:{
                       rowKey:'F_Id',
                   ...
                   created() {
                this.getF_PurReceiptNumber_Option();                      const token = JSON.parse(localStorage.getItem('avue-token'))['content'];
          ...
          getF_PurReceiptNumber_Option(){ 
             request({ 
                 ...
                 this.F_PurReceiptNumber_Option = res.data.data 
             }) 
          }, 
                         download(){
                             request({
                                 ...
                                 responseType: 'blob'
                                    ...
                                    link.href = url;
                                    link.setAttribute('download', 'ERP_PurchaseReturnInfo.xlsx');
                         ...
                         getListData() {
                            request({
                                url: baseUrl + '/erp_purchasereturninfo',
                                method: 'get',
                       ...
                       this.searchParam.F_ReturnNumber = null;
                       this.searchParam.F_ReturnDate_Start = null;
                       this.searchParam.F_ReturnDate_End = null;
                       this.searchParam.F_ReturnDate_Date = null;
                       this.F_ReturnDate_Date = [];
                       this.searchParam.F_PurReceiptNumber = null;
                       this.searchParam.F_ChargePeople = null;
                       this.searchParam.F_InvalidMark = null;
...
                           }
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/erp_purchasereturninfo/" + val.F_Id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                         ...
                         sortChange(data) {
                          ...
                          /* 导出 */
                          tableExport(){
                            this.$refs["ERP_PurchaseReturnInfoTable"].rowExcel()
                            return;
                         },
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/erp_purchasereturninfo/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
                   ...
                   F_ReturnDate_DateChange(val){
                       this.searchParam.F_ReturnDate_Start = val[0];
                       this.searchParam.F_ReturnDate_End = val[1];
                   }
         }
     }
</script>
