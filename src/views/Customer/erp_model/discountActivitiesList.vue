<template>
    ...
    <main-content :title="contentTitle" >
        <div slot="search">
             ...
             <div :class="[!searchFlag ? 'searchBox' : '']">
                <el-form label-width="70px" style="display: inline-block; margin-right:70px;">
                    <el-form-item  label="活动名称" style="display: inline-block;">
						<el-input v-model="searchParam.name" size="small" placeholder="请输入活动名称查询"   style="width:200px" ></el-input>
                    </el-form-item>
                    <el-form-item  label="活动状态" style="display: inline-block;">
						<el-select v-model="searchParam.status" filterable clearable style="width:200px"   placeholder="请选择">
							<el-option  v-for="(item,index) in status_Option"  :key="index" :label="item.F_ItemName"  :value="item.F_ItemValue" >  </el-option>
						</el-select> 
                    </el-form-item>
                    <el-form-item  label="活动时间" style="display: inline-block;">
                        <el-date-picker size="small"  type="datetimerange" value-format="yyyy-MM-dd HH:mm:ss" v-model="startTime_Date"   style="width:320px" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="startTime_DateChange"> </el-date-picker>
                    </el-form-item>
					<el-form-item  label="创建时间" style="display: inline-block; margin-right: 5px;">
					   <el-date-picker size="small"  type="datetimerange" value-format="yyyy-MM-dd HH:mm:ss" v-model="F_CreatorTime_Date"   style="width:320px" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="F_CreatorTime_DateChange"> </el-date-picker>
					</el-form-item>
        ...
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="discountActivitiesTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
                       @sort-change="sortChange" 
                       @current-row-change="handleCurrentRowChange"  @row-dblclick="editForm">
                <template slot-scope="scope" slot="menu">
		  			   ...
		  			   <template v-else-if="scope.row.flowStatus&&scope.row.flowStatus==3">
						  ...
						  <el-button type="opr-primary" size="mini" v-if="hasBtns('modify')&&hasButtonPermission('modify')" @click.stop="editForm(scope.row)">编辑</el-button>
						  <el-button type="opr-primary" size="mini" v-if="hasBtns('modify')&&hasButtonPermission('modify')" @click.stop="viewForm(scope.row)">参与明细</el-button>
						  <el-button type="opr-danger" size="mini" v-if="hasBtns('delete')&&hasButtonPermission('delete')" @click.stop="deleteForm(scope.row)">删除</el-button>
		  			   </template>
		  			   <template v-else>
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <discountActivitiesForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></discountActivitiesForm>
            </el-dialog>
      <el-dialog title="快速导入" width="400px" :append-to-body="true" :visible.sync="excelDialog">
        <el-upload
           ...
           :action="baseUrl + '/discountactivities/import'">
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import discountActivitiesForm from './discountActivitiesForm';
     export default {
         name: "discountActivities",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,discountActivitiesForm,createFlow },
         data() {
			  ...
			  btns:[
                   ...
                   ids: '',
                   startTime_Date : [],                   F_CreatorTime_Date : [],                   listData: [],
                   keyValue: '',
                   flag:false,
             ...
             status_Option : [],                   option:{
                   ...
                   created() {
                this.getstatus_Option();                      const token = JSON.parse(localStorage.getItem('avue-token'))['content'];
                      this.headers.Authorization = 'Bearer ' + token
                       this.uploadData = { F_ModuleId: this.$route.meta.moduleid };
                   },
				   mounted(){
				   	  let themeName = getStore({ name: "themeName" }) || "default";
				   	  if((themeName == "eveningTheme" && window.screen.width<1755)||(themeName != "eveningTheme" &&window.screen.width<1755)){
                   ...
                   methods: {
                         handleCurrentRowChange(row){
                         	this.$refs.discountActivities.toggleSelection([row]);
                         },
                         printForm() {
          ...
          getstatus_Option(){ 
             request({ 
                 ...
                 this.status_Option = res.data.data 
             }) 
          }, 
                         download(){
                             request({
                                 ...
                                 responseType: 'blob'
                                    ...
                                    link.href = url;
                                    link.setAttribute('download', 'discountActivities.xlsx');
                         ...
                         getListData() {
                            request({
                                url: baseUrl + '/discountactivities',
                                method: 'get',
                       ...
                       this.searchParam.startTime_Start = null;
                       this.searchParam.startTime_End = null;
                       this.searchParam.startTime_Date = null;
                       this.startTime_Date = [];
                       this.searchParam.F_CreatorTime_Start = null;
                       this.searchParam.F_CreatorTime_End = null;
                       this.searchParam.F_CreatorTime_Date = null;
                       this.F_CreatorTime_Date = [];
                            this.getListData()
                         },
                          /*查看*/
                         viewForm(val) {
...
                           }
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/discountactivities/" + val.id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                         ...
                         sortChange(data) {
                          ...
                          /* 导出 */
                          tableExport(){
                            this.$refs["discountActivitiesTable"].rowExcel()
                            return;
                         },
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/discountactivities/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
                   ...
                   startTime_DateChange(val){
                       this.searchParam.startTime_Start = val[0];
                       this.searchParam.startTime_End = val[1];
                   },
                   F_CreatorTime_DateChange(val){
                       this.searchParam.F_CreatorTime_Start = val[0];
                       this.searchParam.F_CreatorTime_End = val[1];
                   }
         }
     }
</script>
