<template>
    ...
    <main-content :title="contentTitle" >
             ...
             <div :class="[!searchFlag ? 'searchBox' : '']">
                <el-form label-width="70px" style="display: inline-block; margin-right: 70px;">
                    <el-form-item  label="单据日期" style="display: inline-block;">
                        <el-date-picker size="small"  type="daterange" value-format="yyyy-MM-dd" v-model="F_CreatorTime_Date"   style="width:250px" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" @change="F_CreatorTime_DateChange"> </el-date-picker>
                    </el-form-item>
                    <el-form-item  label="收款编码" style="display: inline-block;">
						<el-input v-model="searchParam.F_ReceiptCode" size="small" placeholder="请输入收款编码查询"   style="width:200px" ></el-input>
                    </el-form-item>
					...
					<el-form-item  label="客户" label-width="50px" style="display: inline-block; margin-right: 5px;">
					   <el-input v-model="searchParam.F_ClientId" size="small" placeholder="请输入客户查询"   style="width:200px" ></el-input>
					</el-form-item>
        ...
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="ERP_FinanceReceiptTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <ERP_FinanceReceiptForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></ERP_FinanceReceiptForm>
            </el-dialog>
        </div>
    </main-content>
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import ERP_FinanceReceiptForm from './ERP_FinanceReceiptForm';
     export default {
         name: "ERP_FinanceReceipt",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,ERP_FinanceReceiptForm,createFlow },
         data() {
			  ...
			  btns:[
				   ...
				   mounted(){
				   	  ...
				   	  if((themeName == "eveningTheme" && window.screen.width<1775)||(themeName != "eveningTheme" &&window.screen.width<1775)){
                   ...
                   methods: {
                         ...
                         getListData() {
                            request({
                                url: baseUrl + '/erp_financereceipt',
                                method: 'get',
                       ...
                       this.searchParam.F_ReceiptCode = null;
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/erp_financereceipt/" + val.F_Id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/erp_financereceipt/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
