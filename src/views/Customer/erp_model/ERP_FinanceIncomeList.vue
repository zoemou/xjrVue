<template>
    ...
    <main-content :title="contentTitle" >
        <div slot="search" >
             ...
             <div class="searchBox">
                <el-form label-width="70px"  style="display: inline-block; margin-right: 70px;">
                    <el-form-item  label="收入编码"  style="display: inline-block;">
						<el-input v-model="searchParam.F_IncomeCode" size="small" placeholder="请输入收入编码查询"   style="width:200px" ></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="业务类型"  style="display: inline-block; margin-right:5px;">
        ...
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="ERP_FinanceIncomeTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <ERP_FinanceIncomeForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></ERP_FinanceIncomeForm>
            </el-dialog>
        </div>
    </main-content>
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import ERP_FinanceIncomeForm from './ERP_FinanceIncomeForm';
     export default {
         name: "ERP_FinanceIncome",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,ERP_FinanceIncomeForm,createFlow },
         data() {
             ...
             F_BusinessType_Option : [],                   option:{
				...
				mounted(){
					  ...
					  if((themeName == "eveningTheme" && window.screen.width<1484)||(themeName != "eveningTheme" &&window.screen.width<1484)){
             ...
             request({ 
                 ...
                 this.F_BusinessType_Option = res.data.data 
                         ...
                         searchClick() {
                          ...
                          /* 查询表单重置 */
                         searchReset() {
                       this.searchParam.F_IncomeCode = null;
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/erp_financeincome/" + val.F_Id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/erp_financeincome/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
