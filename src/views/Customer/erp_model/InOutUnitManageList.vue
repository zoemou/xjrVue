<template>
    ...
    <main-content :title="contentTitle" >
        <div slot="search">
             ...
             <div class="searchBox">
                <el-form label-width="80px" style="display: inline-block;">
                    <el-form-item  label="供应商编号" style="display: inline-block;">
						<el-input v-model="searchParam.F_EnCode" size="small" placeholder="请输入供应商编号查询"   style="width:200px" ></el-input>
                    </el-form-item>
                    <el-form-item  label="供应商名称" style="display: inline-block; margin-right: 5px;">
						<el-input v-model="searchParam.F_FullName" size="small" placeholder="请输入供应商名称查询"   style="width:200px" ></el-input>
                    </el-form-item>
                </el-form>
        ...
        <div slot="table">
        <!-- 在此处放置表格内容 -->
            <avue-crud ref="InOutUnitManageTable" :data="listData" @selection-change="selectionChange" id="printTable" :option="option" stripe :page.sync="page"
                       @on-load="onPageLoad"
            ...
            <el-dialog title="表单" :append-to-body="true" :fullscreen="true" :visible.sync="flag"  :class="isWorkflowForm ? 'work-form-dialog' : ''" :show-close="!isWorkflowForm" >
                <InOutUnitManageForm ref="xtForm"  v-if="flag"  :isEdit="isEdit" :disabled="disabled" @onChange="dialogChange" :keyValue="keyValue"></InOutUnitManageForm>
            </el-dialog>
        </div>
    </main-content>
...
<script>
     ...
     import MainContent from "@/page/main/MainContent";
      ...
      import InOutUnitManageForm from './InOutUnitManageForm';
     export default {
         name: "InOutUnitManage",
         mixins: [authorizeMixin,tableOptionMixin,schemeMixins],
         components: { MainContent, MainFixed,InOutUnitManageForm,createFlow },
         data() {
              return {
					computedHeight:0,
					btns:[
                         ...
                         deleteForm(val) {
                            this.$confirm("确定要删除此项吗？", "提示", {
                              ...
                              type: "warning",
                                 ...
                                 request({
                                     url: baseUrl + "/inoutunitmanage/" + val.F_Id,
                                     method: "delete",
                                 }).then((res) => {
                                     this.getListData()
                                     this.$notify({
                           ...
                           batchDelete() {
                            if (!this.ids || this.ids.split(",").length === 0) {
                             ...
                             this.$confirm("确定要批量删除所选项吗？", "提示", {
                               ...
                               request({
                                 url: baseUrl + "/inoutunitmanage/" + this.ids,
                                 method: "delete",
                               }).then((res) => {
                                 this.getListData();
                                 this.$notify({
