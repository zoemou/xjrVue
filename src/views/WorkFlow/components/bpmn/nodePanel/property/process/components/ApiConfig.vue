          ...
          <el-form :model="iframeData" :rules="rule" ref="formInline" label-width="80px" style="height:100%">
			...
			<template slot="paramValType" slot-scope="scope">
			  <el-select v-model="scope.row.paramValType" :placeholder="$t('selectApiParameterValueType')" @change="changeParamVal(scope.row)">
				...
				<el-option :label="$t('processData')" :value="1"></el-option>
			  </el-select>
			</template>
    ...
    return {
	  ...
	  dic:[{
		  ...
		  children:[{
			...
			label:this.$t('originatorName'),
			value:'username'
		  },{
			label:this.$t('originatorToken'),
			value:'token'
		  },{
			label:this.$t('originatorCompanyId'),
			value:'companyid'
		  },{
			label:this.$t('originatorDepartmentId'),
			value:'departid'
		  },{
			label:this.$t('originatorPostId'),
			value:'postid'
		  },{
			label:this.$t('originatorRoleId'),
			value:'roleid'
		  }]
	  }]
    };
